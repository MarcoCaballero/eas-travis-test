			...
			if (proxies != null) {
				for (int i = 0; i < proxies.length; i++) {
					...
					if (host == null) {
						...
						s.println("<td><a href=" + ogg2m3u(foo) + ">" + foo.substring(host.length() - 1)
								+ "</a>&nbsp;at&nbsp;<a href=" + host + ">" + host + "</a></td>");
					}
					s.println("</tr>");
				}
			}
			...
			if (keys.hasMoreElements()) {
				...
				for (int i = 0; i < Client.clients.size(); i++) {
					...
					if (c instanceof ShoutClient) {
						...
						s.println("<td align=left>");
						s.print("<a href=http://" + sc.dsthost + ":" + sc.dstport + sc.dstmpoint + ">http://"
								+ sc.dsthost + ":" + sc.dstport + sc.dstmpoint + "</a>");
						s.println("</td>");

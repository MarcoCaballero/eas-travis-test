...
class Pls extends Page {
	...
	public void kick(MySocket ms, Hashtable vars, Vector h) throws IOException {
		...
		if (source != null) {
			...
			ms.println("Content-Type: audio/x-scpls");
			ms.println("");
			ms.println("[playlist]");
			ms.println("NumberOfEntries=1");
			ms.println("File1=" + HttpServer.myURL + ogg);
			ms.flush();
			ms.close();
		} else {
			notFound(ms);
	...
	static String getURL(String pls) {

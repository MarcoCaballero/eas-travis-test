...
class Proxy extends Source implements Runnable {
	...
	static final int MAXIMUM_TIME = 600000;
	...
	public void kick() {
		if (me != null) {
			if (System.currentTimeMillis() - lasttime > MAXIMUM_TIME) {
				stop();
			}
			return;
		}
	...
	public void run() {
		...
		try {
			 ...
			 * if(foo.indexOf(" 200 ")==-1){ stop(); return; }
			 */
			...
			while (!eos) {
				...
				while (!eos) {
					...
					if (result == -1) { // missing or corrupt data at this page
						...
						if ((og.granulepos() == 0) || (og.granulepos() == -1) // hack
																				// for
																				// Speex
						) {
							if (header != null) {
						...
						// synchronized(listeners){ // In some case, c.write
						// will block.
